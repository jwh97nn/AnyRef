...
from utils.coco20i import DataCollector, mycoco20i_Tokenized
...
def main(
    ...
    if full_model is None:
        ...
        model = AnyRefForCausalLM.from_pretrained(model_version, torch_dtype=torch.float16, **model_args)
        ...
        model = AnyRefForCausalLM.from_pretrained(
            full_model, low_cpu_mem_usage=True, seg_token_idx=seg_token_idx, torch_dtype=torch.float16,
            vision_pretrained="SAM/sam_vit_h_4b8939.pth", rephrase_weight=0.1,
        )
        model.config.eos_token_id = tokenizer.eos_token_id
        model.config.bos_token_id = tokenizer.bos_token_id
        model.config.pad_token_id = tokenizer.pad_token_id
    ...
    for subdir in subdirs.split(","):
        ...
        for split in [0, 1, 2, 3]:
            ...
            dataloader = DataLoader(dataset, batch_size=1, collate_fn=datacollector, 
                                    # shuffle=True)
                                    shuffle=False)
            ...
            with open(os.path.join(lora_name, "coco", subdir, str(split), "0_result.txt"), "a") as f:
...
if __name__ == "__main__":
    lora_name = "output2/cocoinv+ade+ref+ref_inv/checkpoint-6700"
    ...
    main(
        ...
        seg_start_end=False,
        rephrase_weight=0.1,
        subdirs="mask_test1,mask_test2,mask_test3,mask_test4,mask_test5",
        apply_mask=True,
        # apply_mask=False,
    )
